...
def skyrmion_circularity(
    ...
    ring_mask = np.logical_and(
        np.abs(spins[:, 2] - z_value) <= cutoff_ring,
        np.linalg.norm(positions - center, axis=1) <= max_radius,
    )
    ...
    for i in range(N_Segments):
        ...
        rho_2dgrid = np.array(
            [
                np.array([np.cos(phi), np.sin(phi)]) * rho + center[:2]
                for rho in rho_temp
            ]
        )
        ...
        if i > 0:
            circumference += np.linalg.norm(
                circ_grid[i] - circ_grid[i - 1]
            )  # Increment the circumference
            area += (
                np.linalg.norm(
                    np.cross(
                        np.array([np.cos(phi) * rho, np.sin(phi) * rho]),
                        (circ_grid[i] - circ_grid[i - 1]),
                    )
                )
                / 2
...
def hopfion_normal(
    spin_system, background_direction=[0, 0, 1], background_angle_diff=np.pi / 4
):
...
def hopfion_radius(spin_system, center, normal, z_iso_value=0, z_iso_tolerance=0.1):
    ...
    mask = [
        s[2] > z_iso_value - z_iso_tolerance and s[2] < z_iso_value + z_iso_tolerance
        for s in spins
    ]

    masked_positions = positions[mask]
    radial_directions = masked_positions - center
    projected_radial_directions = np.array(
        [r - r.dot(normal) * normal for r in radial_directions]
    )
...
def compute_pre_image(
    positions, spins, pre_image_spin, angle_tolerance=0.1, n_neighbours=20
):
    ...
    def exclude_indices(
        neighbour_indices, distances, segments, segment_best, distance_best
    ):
        ...
        excluded_indices = neighbour_indices[
            np.logical_and(in_path_direction, less_distant)
        ]
    ...
    angles = np.arccos(
        np.dot(spins, pre_image_spin)
    )  # angles between spins and pre-image-spin
    ...
    neighbour_indices = neighbour_indices[
        1:
    ]  # remove the point itself (distance zero from the distances and the neighbour indices)
    ...
    for i, idx_neigh in enumerate(neighbour_indices):
        segments[i] = (
            positions[idx_neigh] - positions[pre_image[0]]
        )  # segment from idx_first to neighbour_index
        segments[i] /= np.linalg.norm(segments[i])

    # Update exclude indices for first segment
    idx_excluded.extend(
        exclude_indices(
            neighbour_indices, distances, segments, last_segment, distance_best
        )
    )
    ...
    while RUN:
        last_segment = positions[pre_image[-1]] - positions[pre_image[-2]]
        last_segment /= np.linalg.norm(
            last_segment
        )  # direction of the last segment of the pre-image

        distances, neighbour_indices = tree.query(
            positions[pre_image[-1]], k=n_neighbours
        )
        neighbour_indices = neighbour_indices[
            1:
        ]  # remove the point itself (distance zero from the distances and the neighbour indices)
        ...
        if (
            pre_image[0] in neighbour_indices
            and np.dot(last_segment, positions[pre_image[0]] - positions[pre_image[-1]])
            > 0
        ):
        ...
        i_best, idx_neigh_best, segments = find_best_neighbour(
            neighbour_indices, distances, positions, last_segment
        )
