...
def test_gneb_workflow():
    ...
    gnw = gneb_workflow.GNEB_Node(
        name="gneb_test",
        input_file=INPUT_FILE,
        output_folder=OUTPUT_FOLDER,
        initial_chain_file=INITIAL_CHAIN,
    )
    ...
    gnw.clamp_and_refine(
        idx_max_list=[2], max_total_iterations=1e4, convergence=1e-5, apply_ci=False
    )
    gnw.clamp_and_refine(
        idx_max_list=[2],
        max_total_iterations=2e4,
        convergence=1e-8,
        apply_ci=True,
        target_noi=3,
    )
